...
import { useGet, usePut } from "@/hooks/useApiFetch";
...
export default function EditCategoryPage() {
  ...
  const [updateState, updateCategory] = usePut<Category>(`${API_URL}/categories/${id}`, {
  ...
  // Set categories and find the category to edit
  useEffect(() => {
  ...
  }, [categoriesState.data, categoriesState.isSuccess, id, router]);
  
  // Update loading state
  useEffect(() => {
    setLoading(categoriesState.loading || updateState.loading);
  }, [categoriesState.loading, updateState.loading]);
  
  // Handle navigation after successful category update
  useEffect(() => {
    if (updateState.isSuccess) {
      router.push('/home/admin/categories');
    }
  }, [updateState.isSuccess, router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Update the category with auth header
    if (token) {
          ...
          <div className="flex justify-between items-center mb-8">
            <div className="flex items-center">
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Button 
                  variant="ghost" 
                  onClick={() => router.push('/home/admin/categories')}
                  className="mr-4"
                >
